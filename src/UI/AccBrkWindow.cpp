...
AccBrkWindow::AccBrkWindow(UiManager* manager) : m_uiManager(manager) {
    const std::vector<std::string> keys_to_plot = {"ACC1A", "ACC1B", "ACC2A", "ACC2B", "BRK1", "BRK2"};
...
void AccBrkWindow::draw() {
    // Solo per modalità lettura file
    ...
    if (chosen_step < 1.0) chosen_step = 1.0;
    ...
    if (ImPlot::BeginPlot("##Acceleratori", ImVec2(-1, plot_height), ImPlotFlags_NoTitle)) {
        ...
        
        {
            ...
            if (has_data) {
                if (std::abs(max_y - min_y) < 1.0) max_y += 5.0;
            ...
            ImPlot::PlotLine("ACC1A", m_plotData.at("ACC1A").X.data(), m_plotData.at("ACC1A").Y.data(), m_plotData.at("ACC1A").X.size());
    ...
    if (ImPlot::BeginPlot("##Freni", ImVec2(-1, plot_height), ImPlotFlags_NoTitle)) {
...

        {
            ...
            if (has_data) {
                double final_min_y = 800.0, final_max_y = 1000.0;
